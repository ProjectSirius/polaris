...
function calcMarkerMarkerStyle(scale, zIndexStyle, markerStyle, imageStyle) {
  const contrast =
    K_MIN_CONTRAST + (1 - K_MIN_CONTRAST) * Math.min(scale / K_SCALE_NORMAL, 1);
...
export default class MapMarker extends Component {
  ...
  render() {
    ...
    let scale =
      this.props.$hover || this.props.showBallon
        ? K_SCALE_HOVER
        : this.props.scale;
    ...
    const zIndexStyle = {
      zIndex:
        ...
        (this.props.showBallon ? 20 : 0) +
        (this.props.$hover ? K_HINT_HTML_DEFAULT_Z_INDEX : 0), // balloon
    };
    ...
    const hintBaloonHorizontalPosStyle = getHintBaloonHorizontalPosStyle(
      markerDim.x,
    ...
    const hintBalloonBottomOffsetClass = getHintBottomOffsetClass(
      this.props.size.width,
      this.props.origin.x
    );
    ...
    const noTransClass =
      this.props.$hover === true && this.props.hoverState !== true
        ? 'hint--notrans'
        : '';
    const noTransBalloonClass =
      this.props.showBallon === true && this.props.showBallonState !== true
        ? 'hint--notrans'
        : '';
    ...
    return (
      ...
      >
        ...
        >
          {this.props.withText ? (
            <div style={textStyle}>{this.props.marker.get('number')}</div>
          ) : (
            <div />
          )}
        </div>
        <div
          ...
          className={cx(
            'hint__content map-marker-hint',
            this.props.showBallon ? '' : 'noevents'
          )}
        ...
        >
          ...
          <div
            className={cx(
              ...
              this.props.showBallon ? 'map-marker-hint__content--visible' : ''
            )}
          >
          ...
          <div>
            <a
              className={cx(
                ...
                this.props.showBallon ? 'map-marker-hint__ap-link--hidden' : ''
              )}
            >
  ...
  componentDidUpdate(prevProps) {
    ...
    if (prevProps.$hover !== this.props.$hover) {
      setTimeout(
        () => this._onHoverStateChange(this.props.$hover),
        K_TRANS_DELAY
      );
    }
    ...
    if (prevProps.showBallon !== this.props.showBallon) {
      setTimeout(
        () => this._onShowBallonStateChange(this.props.showBallon),
        K_TRANS_DELAY
      );
    }
  }
