...
const Cart = ({
  ...
  return isRequesting ? (
    <Loader active inline="centered" size="huge" className={classes.loading}>
      ...
      <div>
        {cart.length === 0 ? (
          <span className={classes.emptyCartMessage}>{formatMessage(messages.emptyCartMessage)}</span>
        ) : (
            <Container>
              {isGroupOffering ? (
                <Modal
                  ...
                  <List.Item key={item.id}>
                    <List.Content floated="right">
                      <Modal
                        ...
                        centered
                        dimmer
                        trigger={<Button color="twitter">{formatMessage(messages.makeSingleOffer)}</Button>}
                      >
